...
def merge(alma_nlm_merge_df, existing_docline_df):
    ...
    if 'libid' in deleted_output_df.columns:
        deleted_output_df = deleted_output_df.drop(columns=['libid'], errors='ignore')
    
    if 'libid' in add_df.columns:
        add_df = add_df.drop(columns=['libid'], errors='ignore')

    if 'libid' in full_match_output_df.columns:
        full_match_output_df = full_match_output_df.drop(columns=['libid'], errors='ignore')        
    if 'libid' in merged_updated_df.columns:
        merged_updated_df = merged_updated_df.drop(columns=['libid'], errors='ignore')
    if 'libid' in different_ranges_docline_output_df.columns:
        different_ranges_docline_output_df = different_ranges_docline_output_df.drop(columns=['libid'], errors='ignore')
    if 'libid' in different_ranges_alma_output_df.columns:
        different_ranges_alma_output_df = different_ranges_alma_output_df.drop(columns=['libid'], errors='ignore')  

    if 'libid' in in_docline_only_preserve_df.columns:
        in_docline_only_preserve_df = in_docline_only_preserve_df.drop(columns=['libid'], errors='ignore')  
