    ...
    if nlm_choice == "1" or nlm_choice == "2":
        ...
        with open(full_filename, 'rb') as fh:
            ...
            for record in reader:
                ...
                #Joe Ferguson edit
                if record is None:
                    # skip malformed or empty records gracefully
                    continue
                    
                    # existing logic continues under this guard
                
                #print(record['245']['a'])

                #original line: title = record['245']['a']
                #Joe Ferguson edit
                field_245 = record.get_fields('245')
                if field_245:
                    suba = field_245[0].get_subfields('a')
                    title = suba[0].strip(" /:;") if suba else ""
                else:
                    title = ""
                ...
                for oclc_035_tag in record.get_fields('035'):
                    ...
                    for key, value in dict.items():
                        ...
                        for each_value in value:
                            ...
                            if re.search(r"\(OCoLC\)", each_value):
