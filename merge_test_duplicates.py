    ...
    for index, row in df.iterrows():
        if row['record_type'] == "HOLDING":
            ...
            if row['nlm_unique_id'] != current_id or row['holdings_format'] != current_format or row['action'] != current_action:
                ...
                if current_end != 10000 and (row['begin_year'] <= current_end):
...
alma_only_add_df = alma_only_add_df[~alma_only_add_df['begin_year'].isna()]
...
alma_only_add_df = alma_only_add_df.fillna(0)
...
updated_df = updated_df[updated_df.set_index(['nlm_unique_id', 'holdings_format', 'action', 'record_type']).index.isin(merged_new_df_optimized.set_index(['nlm_unique_id', 'holdings_format', 'action', 'record_type']).index)
